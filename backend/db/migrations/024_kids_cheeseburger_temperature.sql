-- Kids Korner Cheeseburger is still a cooked beef burger and needs an explicit
-- doneness choice. Migration 023 correctly scoped temperature choices by full
-- taxonomy coordinates, but incorrectly excluded this legitimate target.
...
BEGIN
  ...
  WHERE choice_group.is_active = true
    ...
    AND NOT (
      ...
      OR (group_record.name = 'Kids' AND category.name = 'Kids Korner' AND item.name = 'Cheeseburger')
    )
  LIMIT 1;
