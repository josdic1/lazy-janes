...
PostgreSQL is the canonical live menu. `backend/src/db/menuImport/menuData.ts` and
`menuOntology.ts` are retained only as one-time import/provenance material for
seeding a brand-new empty database. There is intentionally no `db:sync:menu`
command: manager edits in Menu Management must never be regenerated from the
source snapshot.
